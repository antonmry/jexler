...
On one machine (CentOS), I had the problem that Grab did initially not work, e.g.:

[source,groovy]
@Grab('org.apache.httpcomponents:httpclient:4.4.1')

Apparently, the problem was that Grab tried to access the local Maven repository
(~/.m2/repository) and that there something was missing or only partially there,
in any case I got exceptions with text "Error grabbing grapes".

Creating the following ~/.grape/grapeConfig.xml, according to this
http://stackoverflow.com/questions/6335184/groovy-1-8-grab-fails-unless-i-manually-call-grape-resolve[stackoverflow thread],
...
The following issues seem to be related to the way Tomcat loads classes
and may also depend on Tomcat versions etc.

* Sometimes on Mac OS X and once on CentOS in Tomcat 7, I had the problem
that something within Grape and/or Ivy was apparently not thread safe,
so that autostarting jexlers failed when grabbing dependencies and left
the webapp in a state where also restarting jexlers would not
help any more.
Adding a delay after the startup of each jexler helped to alleviate this issue.
To activate this workaround, set a Java System Property with name
`net.jexler.start.wait.ms` and set it to the time in ms to wait after
each jexler start.

* On CentOS with Tomcat 7, I had the following issue. Several jexlers
used a utility jexler Groovy script which grabbed dependencies via Grape.
The jexlers themselves had no Grape dependencies. During autostart some
classes needed by the utility jexler could often not be found (NoClassDefFoundError),
while starting them manually afterwards worked.
Using the wait time above possibly improved things somewhat; more helpful
was to nominally use Grape within each jexler (see below), although I am still
not sure that this works 100% reliably.

In the utility jexler (Util.groovy):

[source,Groovy]
import groovy.grape.Grape
@Grab('org.apache.httpcomponents:httpclient:4.4.1')
import org.apache.http.client.methods.HttpGet
// more grabs and imports...

In the other jexlers:

[source,Groovy]
----
[ "autostart" : true ]

// don't remove, else may fail to load classes needed by Util class when autostarted
import groovy.grape.Grape
@Grab('org.apache.httpcomponents:httpclient:4.4.1')
// no imports

Util util = new Util(jexler, log)
----
