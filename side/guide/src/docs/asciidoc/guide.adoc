image:jexler.jpg["jexler", link="https://www.jexler.net/"]
...
There is a bug in Groovy/Grape and Ivy up to at least Groovy 2.4.8 / Ivy 2.4.0.
...
By default a workaround is applied, which can be suppressed by setting
the following system property to false:

* `net.jexler.workaround.groovy.7407.grape.engine.wrap`:
  If not set or set to "true", the GrapeEngine in the Grape class will be wrapped,
  so that all Grape calls (@grab etc.) will be synchronized on the Grape.class.
